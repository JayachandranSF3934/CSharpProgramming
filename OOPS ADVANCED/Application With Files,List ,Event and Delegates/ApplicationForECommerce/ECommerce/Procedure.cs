    ...
    public static class Procedure
    {
        ...
        public static void LoginAndPurchase()
       {
        ...
        public static void Purchase()
        {
            ...
            System.Console.WriteLine("Here Are The Details \n");
            ...
            foreach (ProductDetails productOne in productList)
            {
                if(productOne.ProductId == prodId)
                {
                    if(count <= productOne.Stock)
                    {
                        double delivery =50;
                        double totalPrice = (count*productOne.Price) + delivery;

                        if(currentCustomer.WalletBalance >= totalPrice)
                        {
                            currentCustomer.WalletBalance -=totalPrice;
                            productOne.Stock -=count;

                            OrderDetails nextOrder =  new OrderDetails(currentCustomer.CustomerId,prodId,totalPrice,DateTime.Now, count,OrderStatus.Ordered);
                            orderList.Add(nextOrder);    
                            System.Console.WriteLine($"Order Placed successfully. \n Your Order Will be Delivered on {DateTime.Today.AddDays(05)}");   // think properly
                            System.Console.WriteLine($"Your OrderId is {nextOrder.OrderId}");
                        }
                        else{
                    ...
                    else
                    {
                        ...
                        System.Console.WriteLine("Current Availablity is  "+productOne.Stock);
                    }
                }
            }
        }
